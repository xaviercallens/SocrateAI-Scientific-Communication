...
For any level with $\mathrm{genus}(X_0(N))\ge 1$ (for instance $N=11$, and most $N\ge 17$) the
group $\Gamma_0(N)$ has hyperbolic generators, and no soft argument evaluates $w$ there. The reason
is group-theoretic. A character into $\mu_{24}$ factors through the abelianization, and for these
levels $\Gamma_0(N)^{\mathrm{ab}}$ has free factors of infinite order --- concretely, of rank
$2\,\mathrm{genus}(X_0(N))$. On a generator of infinite order the only constraint a torsion
relation such as $w^{24}=1$ can impose is that its image lies in $\mu_{24}$; it does not say
\emph{which} root of unity. Equivalently $\mathrm{Hom}(\Gamma_0(N),\mu_{24})$ is large, and every
element of it is consistent with the relations. Pinning the image therefore requires evaluating the
period integral explicitly, which is what forces Dedekind sums into the argument. At small level the
situation is the opposite and that is exactly why \S\ref{sec:ligozat} succeeds: $\Gamma_0(N)$ is
generated by $-I$, $T$ and $V$, on each of which $w$ \emph{is} pinned. Evaluating $w$ in closed form on a single hyperbolic
...
artifact; see \texttt{BUILDING.md}. Mirrored at
